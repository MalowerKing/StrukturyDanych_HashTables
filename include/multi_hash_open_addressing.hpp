...
const float M = 1.618033;
unsigned int hashFunction(int i) {
        float frac = fmod(i*M, 1.0f);
        return static_cast<unsigned int>(rozmiarTablicy * frac);
};

public:
int* table;

HashTableOpenMulti(unsigned int rozmiar) {
